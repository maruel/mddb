// German translation dictionary.

import type { Dictionary } from "../types";

export const dict: Dictionary = {
  common: {
    loading: "Laden...",
    save: "Speichern",
    saving: "Speichern...",
    saved: "Gespeichert",
    cancel: "Abbrechen",
    delete: "Löschen",
    close: "Schließen",
    back: "Zurück",
    confirm: "Bestätigen",
    actions: "Aktionen",
    invite: "Einladen",
    push: "Übertragen",
    remove: "Entfernen",
    creating: "Erstellen...",
    undo: "Rückgängig",
    redo: "Wiederholen",
  },

  app: {
    title: "mddb",
    tagline: "Ein nahtloses Markdown-basiertes Dokumenten- und Tabellensystem",
    logout: "Abmelden",
    workspace: "Arbeitsbereich",
    newPage: "Neue Seite",
    newTable: "Neue Tabelle",
    settings: "Arbeitsbereich-Einstellungen",
    privacyPolicy: "Datenschutzerklärung",
    terms: "Nutzungsbedingungen",
    myOrg: "Mein Arbeitsbereich",
    createSubPage: "Neue Unterseite",
    createSubTable: "Neue Untertabelle",
    collapseSidebar: "Seitenleiste einklappen",
    collapseNode: "{{title}} einklappen",
    expandNode: "{{title}} ausklappen",
    breadcrumbs: "Brotkrümelnavigation",
    moveNode: "Seite verschieben",
    moveNodeHere: "Seite hierher verschieben",
    moveToRoot: "Seite in den Arbeitsbereich-Stamm verschieben",
    otherWorkspaces: "Andere Arbeitsbereiche",
    switchWorkspace: "Arbeitsbereich wechseln",
    workspaceTree: "Arbeitsbereichsseiten",
    keyboardShortcuts: "Tastenkürzel",
    keyboardShortcutsDescription: "Nutze diese Kürzel außerhalb von Textfeldern und dem Editor.",
    showKeyboardShortcuts: "Tastenkürzel anzeigen",
    focusWorkspaceTree: "Arbeitsbereichsseiten fokussieren",
    moveThroughWorkspacePages: "Arbeitsbereichsseiten durchgehen",
    openFocusedWorkspacePage: "Fokussierte Arbeitsbereichsseite öffnen",
  },

  userMenu: {
    label: "Benutzermenü",
    logout: "Abmelden",
    profile: "Profil",
  },

  profile: {
    title: "Profil",
    myOrganizations: "Meine Organisationen",
    noOrganizations: "Keine Organisationen",
    adminSettings: "Einstellungen",
  },

  auth: {
    login: "Anmelden",
    register: "Registrieren",
    createAccount: "Konto erstellen",
    loginTitle: "Bei mddb anmelden",
    email: "E-Mail",
    password: "Passwort",
    name: "Name",
    pleaseWait: "Bitte warten...",
    alreadyHaveAccount: "Bereits ein Konto?",
    dontHaveAccount: "Noch kein Konto?",
    or: "oder",
    loginWithGoogle: "Mit Google anmelden",
    loginWithMicrosoft: "Mit Microsoft anmelden",
    loginWithGitHub: "Mit GitHub anmelden",
  },

  editor: {
    titlePlaceholder: "Titel",
    contentPlaceholder: "Tippe / für Befehle...",
    unsaved: "Nicht gespeichert",
    history: "Verlauf",
    hideHistory: "Verlauf ausblenden",
    versionHistory: "Versionsverlauf",
    noHistory: "Kein Verlauf verfügbar",
    wysiwygMode: "Visuell",
    markdownMode: "Markdown",
    versionPreview: "Versionsvorschau",
    closePreview: "Vorschau schließen",
    deleteBlock: "Block löschen",
    deleteBlocks: "{{count}} Blöcke löschen",
    duplicateBlock: "Block duplizieren",
    duplicateBlocks: "{{count}} Blöcke duplizieren",
    indent: "Einrücken",
    outdent: "Ausrücken",
    convertTo: "Umwandeln in",
    paragraph: "Text",
    heading: "Überschrift",
    bulletList: "Aufzählung",
    numberedList: "Nummerierte Liste",
    taskList: "Checkliste",
    blockquote: "Zitat",
    codeBlock: "Code-Block",
    addIcon: "Symbol hinzufügen",
    addCover: "Titelbild hinzufügen",
    changeIcon: "Symbol ändern",
    changeCover: "Titelbild ändern",
    removeCover: "Titelbild entfernen",
    removeIcon: "Symbol entfernen",
    iconPickerTitle: "Symbol auswählen",
    iconPickerEmoji: "Emoji",
    iconPickerIcons: "Symbole",
    iconPickerSearch: "Suchen...",
    uploadCover: "Bild hochladen",
  },

  recovery: {
    title: "Etwas ist schiefgelaufen",
    message: "Im Arbeitsbereich ist ein unerwartetes Problem aufgetreten.",
    technicalDetails: "Technische Details",
    retry: "Erneut versuchen",
    reload: "App neu laden",
    copyDetails: "Diagnosedetails kopieren",
    copied: "Diagnosedetails kopiert",
    copyFailed: "Diagnosedetails konnten nicht kopiert werden",
  },

  welcome: {
    title: "Willkommen bei mddb",
    subtitle: "Wählen Sie ein Element aus der Seitenleiste oder erstellen Sie ein neues.",
    createPage: "Seite erstellen",
    createTable: "Tabelle erstellen",
    welcomePageTitle: "Willkommen",
    welcomePageContent:
      "# Willkommen bei mddb\n\nDies ist Ihr neuer Arbeitsbereich. Sie können hier Seiten, Tabellen erstellen und Dateien hochladen.",
  },

  onboarding: {
    welcome: "Willkommen bei mddb!",
    letsGetStarted: "Richten wir Ihren Arbeitsbereich ein.",
    confirmWorkspaceName: "Arbeitsbereichsname bestätigen",
    workspaceName: "Name des Arbeitsbereichs",
    workspaceNameHint: "Sie können diesen Namen ändern.",
    nextGitSync: "Weiter: Git-Synchronisation",
    advancedSyncTitle: "Erweiterte Synchronisation (Optional)",
    advancedSyncHint: "Sichern Sie Ihre Daten durch Synchronisation mit einem privaten Git-Repository.",
    repoUrl: "GitHub/GitLab-Repository-URL",
    repoUrlPlaceholder: "https://github.com/user/repo.git",
    patLabel: "Persönliches Zugriffstoken",
    patPlaceholder: "ghp_...",
    skipForNow: "Vorerst überspringen",
    setupAndFinish: "Einrichten und abschließen",
    defaultOrgName: "{{name}}s Organisation",
    defaultOrgNameFallback: "Meine Organisation",
    defaultWorkspaceName: "{{name}}s Arbeitsbereich",
    defaultWorkspaceNameFallback: "Haupt",
  },

  createOrg: {
    title: "Arbeitsbereich erstellen",
    description: "Erstellen Sie einen neuen Arbeitsbereich für Ihre Inhalte.",
    firstOrgTitle: "Arbeitsbereich erstellen",
    firstOrgDescription: "Willkommen! Richten wir Ihren ersten Arbeitsbereich ein.",
    nameLabel: "Name des Arbeitsbereichs",
    namePlaceholder: "Mein neuer Arbeitsbereich",
    create: "Erstellen",
  },

  createWorkspace: {
    title: "Arbeitsbereich erstellen",
    description: "Erstellen Sie einen neuen Arbeitsbereich für Ihre Inhalte.",
    firstWorkspaceTitle: "Erstellen Sie Ihren ersten Arbeitsbereich",
    firstWorkspaceDescription: "Richten wir Ihren ersten Arbeitsbereich ein.",
    nameLabel: "Name des Arbeitsbereichs",
    namePlaceholder: "Mein neuer Arbeitsbereich",
    create: "Erstellen",
  },

  settings: {
    title: "Einstellungen",
    members: "Mitglieder",
    personal: "Persönlich",
    workspace: "Arbeitsbereich",
    workspaces: "Arbeitsbereiche",
    organizations: "Organisationen",
    gitSync: "Git-Sync",
    // Members tab
    nameColumn: "Name",
    emailColumn: "E-Mail",
    roleColumn: "Rolle",
    adminOnlyMembers: "Nur Administratoren können die Mitgliederliste einsehen.",
    inviteNewMember: "Neues Mitglied einladen",
    emailPlaceholder: "E-Mail-Adresse",
    pendingInvitations: "Ausstehende Einladungen",
    sentColumn: "Gesendet",
    // Personal tab
    personalSettings: "Persönliche Einstellungen",
    theme: "Design",
    themeLight: "Hell",
    themeDark: "Dunkel",
    themeSystem: "System",
    language: "Sprache",
    languageEn: "Englisch",
    languageFr: "Französisch",
    languageDe: "Deutsch",
    languageEs: "Spanisch",
    enableNotifications: "Benachrichtigungen für diesen Arbeitsbereich aktivieren",
    saveChanges: "Änderungen speichern",
    // Linked accounts
    linkedAccounts: "Verknüpfte Konten",
    linkedAccountsHint: "Verbinden Sie zusätzliche Anmeldemethoden mit Ihrem Konto.",
    linkAccount: "Verknüpfen",
    unlinkAccount: "Trennen",
    cannotUnlinkOnly: "Die einzige Authentifizierungsmethode kann nicht getrennt werden",
    linked: "Verknüpft",
    notLinked: "Nicht verknüpft",
    // Email change
    changeEmail: "E-Mail ändern",
    newEmail: "Neue E-Mail-Adresse",
    currentPassword: "Aktuelles Passwort",
    emailChangeHint: "Nach der E-Mail-Änderung müssen Sie diese verifizieren.",
    emailVerified: "Verifiziert",
    emailNotVerified: "Nicht verifiziert",
    // Password management
    addPassword: "Passwort hinzufügen",
    changePassword: "Passwort ändern",
    addPasswordHint: "Fügen Sie ein Passwort hinzu, um die E-Mail/Passwort-Anmeldung zu aktivieren.",
    changePasswordHint: "Ändern Sie Ihr Kontopasswort.",
    newPassword: "Neues Passwort",
    confirmPassword: "Passwort bestätigen",
    passwordMismatch: "Passwörter stimmen nicht überein",
    passwordChanged: "Passwort erfolgreich aktualisiert",
    passwordAdded: "Passwort erfolgreich hinzugefügt",
    // Linking feedback
    accountLinked: "Konto erfolgreich verknüpft",
    accountUnlinked: "Konto erfolgreich getrennt",
    linkingFailed: "Verknüpfung des Kontos fehlgeschlagen",
    // Workspace tab
    workspaceSettings: "Arbeitsbereich-Einstellungen",
    adminOnlyWorkspace: "Nur Administratoren können Arbeitsbereich-Einstellungen ändern.",
    organizationName: "Organisationsname",
    workspaceName: "Arbeitsbereichsname",
    clickToEditName: "Klicken, um Namen zu bearbeiten",
    allowPublicAccess: "Öffentlichen Zugriff erlauben (Nur Lesen)",
    allowedDomains: "Erlaubte E-Mail-Domains (kommagetrennt)",
    allowedDomainsPlaceholder: "beispiel.de, firma.org",
    allowedDomainsHint: "Benutzer mit diesen E-Mail-Domains können automatisch beitreten.",
    saveWorkspaceSettings: "Arbeitsbereich-Einstellungen speichern",
    // Git sync tab
    gitSynchronization: "Git-Synchronisation",
    gitSyncHint: "Synchronisieren Sie Ihre Arbeitsbereichsdaten mit einem externen Git-Repository.",
    urlColumn: "URL",
    lastSyncColumn: "Letzte Sync",
    never: "Nie",
    addNewRemote: "Neues Remote hinzufügen",
    remoteName: "Remote-Name",
    remoteNamePlaceholder: "origin",
    repositoryUrl: "Repository-URL",
    repositoryUrlPlaceholder: "https://github.com/user/repo.git",
    personalAccessToken: "Persönliches Zugriffstoken (optional)",
    tokenPlaceholder: "ghp_...",
    tokenHint: "Wird zur Authentifizierung beim Push verwendet.",
    addRemote: "Remote hinzufügen",
    confirmRemoveRemote: "Sind Sie sicher, dass Sie dieses Remote entfernen möchten?",
    // GitHub App sync
    connectGitHub: "GitHub verbinden",
    selectRepository: "Repository auswählen",
    gitHubAppSetup: "GitHub App",
    manualSetup: "Manuelle Einrichtung",
    selectInstallation: "Installation auswählen",
    gitHubAppNotConfigured: "GitHub App ist auf diesem Server nicht konfiguriert",
    autoPush: "Automatisch übertragen",
    autoPushHint: "Änderungen nach jeder Bearbeitung automatisch zum Remote übertragen.",
    pull: "Abrufen",
    syncStatusIdle: "Bereit",
    syncStatusSyncing: "Synchronisiere...",
    syncStatusError: "Synchronisierungsfehler",
    syncStatusConflict: "Konflikt",
    conflictMessage: "Merge-Konflikt erkannt. Bitte manuell lösen oder erneut abrufen.",
    branchColumn: "Branch",
    statusColumn: "Status",
    // Roles
    roleAdmin: "Administrator",
    roleEditor: "Bearbeiter",
    roleViewer: "Betrachter",
    roleOwner: "Eigentümer",
    roleMember: "Mitglied",
    // Organization settings
    organizationSettings: "Organisationseinstellungen",
    organizationMembers: "Organisationsmitglieder",
    organizationPreferences: "Organisationseinstellungen",
    organizationQuotas: "Organisationskontingente",
    maxWorkspacesPerOrg: "Max. Arbeitsbereiche",
    maxMembersPerOrg: "Max. Mitglieder (Org)",
    maxMembersPerWorkspace: "Max. Mitglieder (Arbeitsbereich)",
    maxTotalStorageBytes: "Max. Speicher gesamt (Bytes)",
    maxTablesPerWorkspace: "Max. Tabellen pro Arbeitsbereich",
    maxColumnsPerTable: "Max. Spalten pro Tabelle",
    // Workspace quotas
    workspaceQuotas: "Arbeitsbereich-Kontingente",
    maxPages: "Max. Seiten",
    maxStorageBytes: "Max. Speicher (Bytes)",
    maxRecordsPerTable: "Max. Datensätze pro Tabelle",
    maxAssetSizeBytes: "Max. Asset-Größe (Bytes)",
    serverCeiling: "Server-Obergrenze",
    parentCeiling: "Übergeordnete Obergrenze",
    inheritFromParent: "Vom übergeordneten erben",
    quotas: "Kontingente",
    orgSettingsHint:
      "Für organisationsweite Einstellungen wie erlaubte E-Mail-Domains gehen Sie zu den Organisationseinstellungen.",
    openOrgSettings: "Organisationseinstellungen öffnen",
    adminOnlySettings: "Nur Administratoren können Organisationseinstellungen ändern.",
    settings: "Einstellungen",
    actionsColumn: "Aktionen",
    confirmRemoveMember:
      "Möchten Sie dieses Mitglied wirklich entfernen? Es verliert den Zugriff auf alle Arbeitsbereiche dieser Organisation.",
  },

  table: {
    records: "Tabellen-Datensätze",
    table: "Tabelle",
    grid: "Raster",
    list: "Liste",
    gallery: "Galerie",
    board: "Kanban-Board",
    noRecords: "Keine Datensätze",
    noColumns: "Keine Spalten. Klicken Sie auf +, um eine Spalte hinzuzufügen.",
    loadMore: "Mehr laden",
    deleteRecord: "Datensatz löschen",
    confirmDeleteRecord: "Diesen Datensatz löschen?",
    confirmDeleteView: "Diese Ansicht löschen?",
    untitled: "Ohne Titel",
    noImage: "Kein Bild",
    noGroup: "Keine Gruppe",
    addSelectColumn: "Fügen Sie eine Auswahlspalte hinzu, um die Board-Ansicht zu nutzen",
    newView: "Neue Ansicht",
    defaultView: "Standard",
    all: "Alle",
    addColumn: "Spalte hinzufügen",
    addRecord: "Datensatz hinzufügen",
    addColumnFirst: "Zuerst eine Spalte hinzufügen",
    columnName: "Spaltenname",
    columnType: "Spaltentyp",
    typeText: "Text",
    typeNumber: "Zahl",
    typeCheckbox: "Kontrollkästchen",
    typeDate: "Datum",
    typeSelect: "Auswahl",
    typeUser: "Benutzer",
    userRemoved: "entfernt",
    typeUrl: "URL",
    typeEmail: "E-Mail",
    sort: "Sortieren",
    filter: "Filtern",
    addSort: "Sortierung hinzufügen",
    noSorts: "Keine Sortierung",
    ascending: "Aufsteigend",
    descending: "Absteigend",
    openRecord: "Öffnen",
    duplicateRecord: "Duplizieren",
    renameView: "Ansicht umbenennen",
    duplicateView: "Ansicht duplizieren",
    renameColumn: "Umbenennen",
    sortAscending: "Aufsteigend sortieren",
    sortDescending: "Absteigend sortieren",
    insertColumnLeft: "Links einfügen",
    insertColumnRight: "Rechts einfügen",
    deleteColumn: "Spalte löschen",
    confirmDeleteColumn: "Diese Spalte und alle Daten löschen?",
    removeSort: "Sortierung entfernen",
    filterBy: "Filtern nach...",
    removeFilter: "Filter entfernen",
    filterApply: "Anwenden",
    filterValue: "Wert",
    opEquals: "ist",
    opNotEquals: "ist nicht",
    opContains: "enthält",
    opNotContains: "enthält nicht",
    opStartsWith: "beginnt mit",
    opEndsWith: "endet mit",
    opIsEmpty: "ist leer",
    opIsNotEmpty: "ist nicht leer",
    opGt: ">",
    opGte: ">=",
    opLt: "<",
    opLte: "<=",
    typeMultiSelect: "Mehrfachauswahl",
    typePhone: "Telefon",
    hideColumn: "Spalte ausblenden",
    showColumn: "Einblenden",
    hiddenColumns: "ausgeblendet",
    recordCount: "Einträge",
    groupBy: "Gruppieren nach",
    recordDetail: "Datensatzdetails",
    searchOptions: "Optionen suchen…",
    editOptions: "Optionen bearbeiten",
    addOption: "Option hinzufügen",
    optionPlaceholder: "Optionsname",
    deleteOption: "Option löschen",
    optionUsedWarning: "Verwendet von {n} Eintrag/Einträgen",
    calendarNotAvailable: "Die Kalenderansicht ist noch nicht verfügbar.",
  },

  errors: {
    VALIDATION_FAILED: "Die angegebenen Daten sind ungültig",
    MISSING_FIELD: "Ein Pflichtfeld fehlt",
    INVALID_FORMAT: "Das Datenformat ist ungültig",
    NOT_FOUND: "Die angeforderte Ressource wurde nicht gefunden",
    NODE_NOT_FOUND: "Knoten nicht gefunden",
    TABLE_NOT_FOUND: "Tabelle nicht gefunden",
    FILE_NOT_FOUND: "Datei nicht gefunden",
    STORAGE_ERROR: "Ein Speicherfehler ist aufgetreten",
    INTERNAL_ERROR: "Ein interner Serverfehler ist aufgetreten",
    NOT_IMPLEMENTED: "Diese Funktion ist noch nicht verfügbar",
    CONFLICT: "Ein Konflikt mit dem aktuellen Zustand ist aufgetreten",
    UNAUTHORIZED: "Bitte melden Sie sich an, um fortzufahren",
    FORBIDDEN: "Sie haben keine Berechtigung für diese Aktion",
    unknown: "Ein unerwarteter Fehler ist aufgetreten",
    sessionExpired: "Sitzung abgelaufen",
    titleRequired: "Titel ist erforderlich",
    failedToLoad: "Laden fehlgeschlagen",
    failedToSave: "Speichern fehlgeschlagen",
    failedToDelete: "Löschen fehlgeschlagen",
    failedToCreate: "Erstellen fehlgeschlagen",
    failedToSwitch: "Arbeitsbereichswechsel fehlgeschlagen",
    failedToInvite: "Einladung senden fehlgeschlagen",
    failedToUpdateRole: "Rollenaktualisierung fehlgeschlagen",
    failedToAddRemote: "Remote hinzufügen fehlgeschlagen",
    failedToRemoveRemote: "Remote entfernen fehlgeschlagen",
    failedToMove: "Verschieben fehlgeschlagen",
    pushFailed: "Push fehlgeschlagen",
    pullFailed: "Pull fehlgeschlagen",
    autoSaveFailed: "Automatisches Speichern fehlgeschlagen",
    noAccessToOrg: "Sie haben keinen Zugriff auf diese Organisation",
    noAccessToWs: "Sie haben keinen Zugriff auf diesen Arbeitsbereich",
    failedToRemoveMember: "Mitglied entfernen fehlgeschlagen",
  },

  success: {
    invitationSent: "Einladung erfolgreich gesendet",
    roleUpdated: "Rolle aktualisiert",
    personalSettingsSaved: "Persönliche Einstellungen gespeichert",
    workspaceSettingsSaved: "Arbeitsbereich-Einstellungen gespeichert",
    orgSettingsSaved: "Organisationseinstellungen gespeichert",
    remoteAdded: "Remote hinzugefügt",
    remoteRemoved: "Remote entfernt",
    pushSuccessful: "Push erfolgreich",
    pullSuccessful: "Pull erfolgreich",
    gitHubAppConfigured: "GitHub App Remote konfiguriert",
    memberRemoved: "Mitglied entfernt",
  },

  pwa: {
    installTitle: "mddb installieren",
    installMessage: "Fügen Sie mddb zu Ihrem Startbildschirm hinzu",
    installButton: "Installieren",
    dismissButton: "Nicht jetzt",
  },

  notionImport: {
    title: "Aus Notion importieren",
    description: "Erstellen Sie einen neuen Arbeitsbereich aus Ihren Notion-Daten.",
    notionToken: "Internes Integrationsgeheimnis",
    notionTokenPlaceholder: "ntn_... oder secret_...",
    setupTitle: "Einrichtungsanleitung:",
    step1Header: "Auf der ersten Seite:",
    step1a: 'Name "mddb import" eingeben',
    step1b: "Arbeitsbereich zum Importieren auswählen",
    step1c: 'Typ "Intern" auswählen',
    step1d: "Auf Speichern klicken",
    step2Header: "Im Tab Zugriff:",
    step2a: "Auf Zugriff bearbeiten klicken",
    step2b: "Die zu importierenden Seiten auswählen",
    step3Header: "Im Tab Konfiguration:",
    step3a: '"Inhalte aktualisieren" und "Inhalte einfügen" deaktivieren',
    step3b: 'Neben "Internes Integrationsgeheimnis" auf "Anzeigen" klicken und unten einfügen',
    createIntegration: "Notion-Integration erstellen",
    startImport: "Import starten",
    importing: "Importiere...",
    cancel: "Import abbrechen",
    statusRunning: "Läuft",
    statusCompleted: "Abgeschlossen",
    statusFailed: "Fehlgeschlagen",
    statusCancelled: "Abgebrochen",
    importInProgress: "Notion-Import läuft...",
    importCompleted: "Notion-Import abgeschlossen!",
    importFailed: "Notion-Import fehlgeschlagen",
    importCancelled: "Notion-Import abgebrochen",
    pagesImported: "Seiten",
    databasesImported: "Datenbanken",
    recordsImported: "Datensätze",
    assetsImported: "Dateien",
    errorsEncountered: "Fehler",
    dismiss: "Schließen",
  },
  slashMenu: {
    noResults: "Keine Ergebnisse",
    paragraph: "Absatz",
    heading1: "Überschrift 1",
    heading2: "Überschrift 2",
    heading3: "Überschrift 3",
    bulletList: "Aufzählung",
    orderedList: "Nummerierte Liste",
    taskList: "Aufgabenliste",
    blockquote: "Zitat",
    codeBlock: "Codeblock",
    divider: "Trennlinie",
    subpage: "Unterseite",
    untitledSubpage: "Ohne Titel",
  },

  notifications: {
    title: "Benachrichtigungen",
    markAllRead: "Alle als gelesen markieren",
    empty: "Keine Benachrichtigungen",
    loadMore: "Mehr laden",
    enablePush: "Browser-Benachrichtigungen aktivieren",
    pushDenied: "Browser-Benachrichtigungen sind blockiert. Aktivieren Sie sie in den Browser-Einstellungen.",
    pushEnabled: "Browser-Benachrichtigungen aktiviert",
    preferences: "Benachrichtigungseinstellungen",
    channelEmail: "E-Mail",
    channelWeb: "Web",
    typeOrgInvite: "Organisationseinladungen",
    typeWsInvite: "Arbeitsbereich-Einladungen",
    typeMemberJoined: "Mitglied beigetreten",
    typeMemberRemoved: "Mitglied entfernt",
    typePageMention: "Seitenerwähnungen",
    typePageEdited: "Seitenänderungen",
    timeJustNow: "gerade eben",
    timeMinutesAgo: "vor {{count}} Min.",
    timeHoursAgo: "vor {{count}} Std.",
    timeDaysAgo: "vor {{count}} T.",
  },

  server: {
    title: "Server-Einstellungen",
    serverSettings: "Server-Einstellungen",
    smtpConfiguration: "SMTP-Konfiguration",
    smtpHost: "SMTP-Server",
    smtpPort: "SMTP-Port",
    smtpUsername: "Benutzername",
    smtpPassword: "Passwort",
    smtpPasswordHint: "Leer lassen um bestehendes Passwort zu behalten",
    smtpFrom: "Absenderadresse",
    quotas: "Kontingente",
    maxRequestBodyBytes: "Max. Anfragegröße (Bytes)",
    maxSessionsPerUser: "Max. Sitzungen pro Benutzer",
    maxTablesPerWorkspace: "Max. Tabellen pro Arbeitsbereich",
    maxColumnsPerTable: "Max. Spalten pro Tabelle",
    maxRecordsPerTable: "Max. Datensätze pro Tabelle",
    maxPages: "Max. Seiten",
    maxStorageBytes: "Max. Speicher (Bytes)",
    maxOrganizations: "Max. Organisationen",
    maxWorkspaces: "Max. Arbeitsbereiche",
    maxUsers: "Max. Benutzer",
    maxTotalStorageBytes: "Max. Speicher gesamt (Bytes)",
    maxAssetSizeBytes: "Max. Asset-Größe (Bytes)",
    maxEgressBandwidthBps: "Max. Ausgangs-Bandbreite (Bytes/Sek)",
    rateLimits: "Ratenlimits",
    rateLimitsHint: "0 bedeutet unbegrenzt. Änderungen werden sofort wirksam.",
    authRatePerMin: "Auth-Rate (Anf./Min)",
    writeRatePerMin: "Schreib-Rate (Anf./Min)",
    readAuthRatePerMin: "Lese-Rate auth. (Anf./Min)",
    readUnauthRatePerMin: "Lese-Rate unauth. (Anf./Min)",
    saveConfiguration: "Konfiguration speichern",
    configurationSaved: "Konfiguration erfolgreich gespeichert",
    smtpEnabled: "SMTP aktiviert",
    smtpDisabled: "SMTP deaktiviert",
    dashboard: "Dashboard",
    totalUsers: "Benutzer gesamt",
    totalOrganizations: "Organisationen gesamt",
    totalWorkspaces: "Arbeitsbereiche gesamt",
    totalStorage: "Speicher gesamt",
    activeSessions: "Aktive Sitzungen",
    organizationName: "Organisation",
    workspaceName: "Arbeitsbereich",
    members: "Mitglieder",
    pages: "Seiten",
    storage: "Speicher",
    created: "Erstellt",
    gitCommits: "Git-Commits",
    requestMetrics: "Anfrage-Metriken",
    serverUptime: "Server-Laufzeit",
    authRequests: "Auth-Anfragen",
    writeRequests: "Schreib-Anfragen",
    readAuthRequests: "Lese-Anfragen (auth.)",
    readUnauthRequests: "Lese-Anfragen (unauth.)",
    reqPerMin: "Anf./Min",
    refresh: "Aktualisieren",
  },

  sse: {
    externalChange: "Diese Seite wurde von einem anderen Benutzer bearbeitet.",
    refreshContent: "Aktualisieren",
    dismissNotice: "Ausblenden",
    connected: "Verbunden",
    reconnecting: "Verbindung wird wiederhergestellt",
  },
  voice: {
    assistant: "Assistent:",
    cancel: "Abbrechen",
    cancelConnection: "Verbindung abbrechen",
    clearTranscript: "Transkript löschen",
    connect: "Sprachassistent verbinden",
    connectionFailed: "Sprachverbindung fehlgeschlagen. Versuchen Sie es erneut.",
    endSession: "Sprachsitzung beenden",
    listening: "Hört zu…",
    microphone: "Mikrofon",
    mute: "Stummschalten",
    muted: "Stummgeschaltet",
    reconnecting: "Verbindung wird wiederhergestellt…",
    retry: "Erneut versuchen",
    signaling: "Verbindung wird aufgebaut…",
    speaker: "Lautsprecher",
    speaking: "Spricht…",
    transcript: "Transkript",
    transcriptPlaceholder: "Das Gespräch erscheint hier",
    unmute: "Stummschaltung aufheben",
    voiceAssistant: "Sprachassistent",
    waitingForServer: "Warten auf Server…",
    settingUpWebRTC: "WebRTC wird eingerichtet…",
    you: "Sie:",
    setupFailed: "Sprachassistent nicht verfügbar. Laden Sie die Seite erneut.",
  },
};
