...
Separate structs used for disk serialization from those used for API responses to improve type safety, clarity, and maintainability.

#### Problem Statement

Currently, `models.go` defines structs that serve dual purposes:
1. **Disk persistence**: Serialized to JSONL files in `data/db/`
2. **API responses**: Returned directly from HTTP handlers

This conflation causes several issues:

| Issue | Example | Risk |
|-------|---------|------|
| Runtime fields in disk models | `User.Memberships`, `User.OrganizationID`, `User.Role` | Stale data accidentally persisted |
| Sensitive fields in API models | `Invitation.Token` returned in list responses | Security exposure |
| No type-level enforcement | Developer must remember which fields are runtime-only | Bugs from misuse |
| Inconsistent API responses | Some handlers format timestamps, others return raw `time.Time` | Frontend complexity |

#### Current State

**Existing separation pattern** (in `storage/user_service.go`):
```go
type userStorage struct {
    models.User                // Embedded domain model
    PasswordHash string        // Storage-only field
}
```

**Fields that are runtime-only** (marked `omitempty`, populated by handlers):
- `User.Memberships` - loaded via `populateMemberships()`
- `User.OrganizationID` - set in `PopulateActiveContext()`
- `User.Role` - set in `PopulateActiveContext()`
- `User.Onboarding` - set in `PopulateActiveContext()`
- `Membership.OrganizationName` - joined from Organization table

**Fields that should not appear in API responses**:
- `Invitation.Token` - secret verification token
...
┌─────────────────────────────────────────────────────────────┐
│                      API Layer (api.go)                     │
│  UserResponse, MembershipResponse, InvitationResponse, ...  │
│  - Explicit fields for API consumers                        │
│  - Formatted timestamps (string, not time.Time)             │
│  - Computed/joined fields included                          │
...
│  userStorage, invitationStorage, ...                        │
│  - Embeds domain model                                      │
│  - Adds storage-only fields (PasswordHash, internal IDs)    │
│  - Lives in storage package, not exported                   │
└─────────────────────────────────────────────────────────────┘
```

#### Detailed Design

**1. Domain Models (`models/models.go`)** - Trimmed to persistent fields only:
...
type Membership struct {
    ...
    Created        time.Time          `json:"created"`
    // Note: OrganizationName removed - it's a joined field for API only
}

// Invitation - Token field remains for storage but excluded from API.
type Invitation struct {
    ID             jsonldb.ID `json:"id"`
    Email          string     `json:"email"`
    OrganizationID jsonldb.ID `json:"organization_id"`
    Role           UserRole   `json:"role"`
    Token          string     `json:"token"`      // Stored, but not in API response
    ExpiresAt      time.Time  `json:"expires_at"`
    Created        time.Time  `json:"created"`
}
```

**2. API Response Models (`models/api.go`)** - Explicit response types:
...
type UserResponse struct {
    ...
    OAuthIdentities []OAuthIdentity      `json:"oauth_identities,omitempty"`
    Settings        UserSettings         `json:"settings"`
    Created         string               `json:"created"`  // ISO8601 formatted
    Modified        string               `json:"modified"` // ISO8601 formatted

    // Context fields (populated based on request context)
...
type MembershipResponse struct {
    ...
    OrganizationName string             `json:"organization_name"` // Joined field
    Role             UserRole           `json:"role"`
    Settings         MembershipSettings `json:"settings"`
    Created          string             `json:"created"`
}

// InvitationResponse excludes the secret token.
...
// ToAPI converts a domain User to an API response.
func (u *User) ToAPI() *UserResponse {
    return &UserResponse{
        ID:              u.ID.String(),
        Email:           u.Email,
        Name:            u.Name,
...
**3. Storage Models (`storage/`)** - Already established pattern, extend as needed:

```go
// In storage/user_service.go (existing)
type userStorage struct {
    models.User
    PasswordHash string `json:"password_hash"`
}

// In storage/invitation_service.go (if sensitive fields grow)
// Currently Invitation can stay as-is since Token is needed for storage
// and we just don't expose it in ToAPI()
```

#### Migration Strategy

1. **Phase A: Add API response types** (non-breaking)
   - Add `UserResponse`, `MembershipResponse`, `InvitationResponse` to `api.go`
   - Add `ToAPI()` methods to domain models
   - Update handlers to use `ToAPI()` instead of returning domain models directly

2. **Phase B: Remove runtime fields from domain models** (breaking for internal code)
   - Remove `User.Memberships`, `User.OrganizationID`, `User.Role`, `User.Onboarding`
   - Remove `Membership.OrganizationName`
   - Update all internal code to populate API response types instead

3. **Phase C: Standardize timestamp formatting**
   - All API responses use RFC3339 string timestamps
   - All domain models use `time.Time` internally

#### Affected Structs Analysis

| Struct | Disk-only Fields | Runtime-only Fields | Action |
|--------|------------------|---------------------|--------|
| `User` | (via userStorage: PasswordHash) | Memberships, OrganizationID, Role, Onboarding | Create `UserResponse` |
| `Membership` | - | OrganizationName | Create `MembershipResponse` |
| `Organization` | - | - | Minimal: just add `ToAPI()` for timestamp formatting |
| `Invitation` | Token | - | Create `InvitationResponse` (excludes Token) |
| `GitRemote` | (via remoteSecret: Token) | - | Create `GitRemoteResponse` for consistency |
| `Node` | - | Children (loaded on demand) | Create `NodeResponse` |
| `DataRecord` | - | - | Clean, possibly just timestamp formatting |

#### Implementation Tasks

- [ ] **API Response Types**
    - [ ] Define `UserResponse` with `ToAPI()` conversion method
    - [ ] Define `MembershipResponse` with `ToAPI()` conversion method
    - [ ] Define `InvitationResponse` (excludes Token) with `ToAPI()` conversion method
    - [ ] Define `OrganizationResponse` with `ToAPI()` conversion method
    - [ ] Define `GitRemoteResponse` with `ToAPI()` conversion method
    - [ ] Define `NodeResponse` with `ToAPI()` conversion method

- [ ] **Handler Updates**
    - [ ] Update `AuthHandler` to return `UserResponse` instead of `*User`
    - [ ] Update `UserHandler` to return `UserResponse` / `[]UserResponse`
    - [ ] Update `InvitationHandler` to return `InvitationResponse`
    - [ ] Update `MembershipHandler` to return `MembershipResponse`
    - [ ] Update `OrganizationHandler` to return `OrganizationResponse`
    - [ ] Update `NodeHandler` to return `NodeResponse`

- [ ] **Domain Model Cleanup**
    - [ ] Remove `User.Memberships` field (runtime-only)
    - [ ] Remove `User.OrganizationID` field (runtime-only)
    - [ ] Remove `User.Role` field (runtime-only)
    - [ ] Remove `User.Onboarding` field (runtime-only)
    - [ ] Remove `Membership.OrganizationName` field (runtime-only)

- [ ] **Context Population Refactor**
    - [ ] Refactor `PopulateActiveContext()` to return `UserResponse` with context fields
    - [ ] Refactor `populateMemberships()` to return `[]MembershipResponse`

- [ ] **Frontend Updates**
    - [ ] Update TypeScript types (auto-generated via tygo)
    - [ ] Verify all API consumers handle new response structure

- [ ] **Testing**
    - [ ] Add tests verifying sensitive fields not exposed in API responses
    - [ ] Add tests for `ToAPI()` conversion methods
    - [ ] Verify JSON serialization matches expected format

#### Benefits

1. **Type Safety**: Compiler prevents accidentally persisting runtime fields or exposing sensitive data
2. **Clear Contracts**: API consumers see explicit types with documented fields
3. **Consistent Formatting**: All timestamps as RFC3339 strings, all IDs as strings
4. **Security**: Sensitive fields (Token, PasswordHash) structurally excluded from responses
5. **Maintainability**: Clear separation of concerns between persistence and presentation
