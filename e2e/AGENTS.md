...
- `all-column-types-views.spec.ts`: E2E screenshot test: table with every column type shown in all four view modes.
- `auth-storage.spec.ts`: Verifies that a token change in another tab replaces the visible account and workspace.
- `block-context-menu.spec.ts`: E2E tests for block context menu interactions.
- `block-drag-reorder.spec.ts`: E2E tests for block drag-and-drop reordering functionality.
- `block-editor.spec.ts`: E2E tests for flat block editor: input rules, drag handles, context menu, keyboard navigation.
- `block-handle-diagnostic.spec.ts`: Diagnostic tests for block handle visibility and text alignment issues.
- `block-handle-visibility.spec.ts`: Test for block handle visibility bug fix.
- `copy-paste.spec.ts`: E2E tests for copy and paste functionality in the block editor.
- `editor-toolbar.spec.ts`: E2E tests for editor toolbar formatting buttons.
- `error-handling.spec.ts`: E2E tests for invalid route handling and error pages.
- `floating-toolbar.spec.ts`: E2E tests for floating toolbar visibility lifecycle.
- `gomode-mcp.spec.ts`: Verifies Go Mode discovery and authenticated MCP reads against a real workspace.
- `helpers.ts`: Shared e2e test helpers for registration, workspace management, and screenshots.
- `inject-tag-colors.cjs`: Inject custom tag colors and server log link into Playwright HTML report.
- `interaction-primitives.spec.ts`: End-to-end coverage for dialog-contained field dropdowns and import-dialog focus behavior.
- `keyboard-shortcuts.spec.ts`: End-to-end coverage for discoverable and editor-safe workspace keyboard shortcuts.
- `language-switch.spec.ts`: E2E tests for language switching in settings.
- `mobile-ui.spec.ts`: E2E tests for mobile-responsive sidebar and UI.
- `oauth.spec.ts`: E2E tests for OAuth2 authentication flow.
- `page-crud.spec.ts`: E2E tests for page create, read, update, and delete operations.
- `page-hierarchy.spec.ts`: E2E tests for page parent-child hierarchy and nesting.
- `page-links.spec.ts`: E2E tests for page links with dynamic title resolution.
- `register-redirect.spec.ts`: E2E test for registration redirect: after registering, user should land on onboarding, not stay on login.
- `remove-org-member.spec.ts`: E2E tests for removing an organization member with cascading workspace membership removal.
- `rename-race.spec.ts`: E2E tests for page rename race conditions during navigation.
- `responsive-shell.spec.ts`: End-to-end coverage for responsive workspace and settings sidebar behavior.
- `select-options.spec.ts`: E2E tests for select/multi_select option management via SelectOptionsEditor.
- `settings.spec.ts`: E2E tests for user settings and language switching.
- `table-operations.spec.ts`: E2E tests for table creation, record CRUD, view modes, and sort UI.
- `task-cursor-position.spec.ts`: E2E test for task block cursor positioning when navigating with arrow keys.
- `view-query-param.spec.ts`: E2E tests for navigating to a table with a specific ?view= query parameter.
- `views-api.spec.ts`: E2E tests for table views API: filters, sorts, and view CRUD.
- `workspace-navigation.spec.ts`: End-to-end keyboard navigation coverage for the workspace sidebar tree.
- `workspace-org.spec.ts`: E2E tests for workspace and organization creation flow.
- `workspace-switch-loop.spec.ts`: E2E test for workspace switch 404 loop bug.
<!-- END FILE INDEX -->
