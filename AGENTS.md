...
- `e2e/all-column-types-views.spec.ts`: E2E screenshot test: table with every column type shown in all four view modes.
...
- `e2e/copy-paste.spec.ts`: E2E tests for copy and paste functionality in the block editor.
...
- `e2e/select-options.spec.ts`: E2E tests for select/multi_select option management via SelectOptionsEditor.
- `e2e/table-operations.spec.ts`: E2E tests for table creation, record CRUD, view modes, and sort UI.
- `e2e/task-cursor-position.spec.ts`: E2E test for task block cursor positioning when navigating with arrow keys.
- `e2e/view-query-param.spec.ts`: E2E tests for navigating to a table with a specific ?view= query parameter.
- `e2e/views-api.spec.ts`: E2E tests for table views API: filters, sorts, and view CRUD.
