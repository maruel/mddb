...
func (h *InvitationHandler) CreateInvitation(ctx context.Context, req models.CreateInvitationRequest) (*models.Invitation, error) {
	...
	if err != nil {
		return nil, errors.InternalWithError("Failed to create invitation", err)
	}
...
func (h *InvitationHandler) AcceptInvitation(ctx context.Context, req models.AcceptInvitationRequest) (*models.LoginResponse, error) {
	if req.Token == "" {
		return nil, errors.MissingField("token")
	}

	inv, err := h.invService.GetInvitationByToken(req.Token)
	if err != nil {
		return nil, errors.NewAPIError(404, errors.ErrNotFound, "Invitation not found or expired")
	}

	if time.Now().After(inv.ExpiresAt) {
		return nil, errors.NewAPIError(400, "Invitation expired", "Invitation expired")
	}
	...
	if err != nil {
		...
		if req.Password == "" || req.Name == "" {
			return nil, errors.MissingField("password and name required for new account")
		}
	...
	if err != nil {
		return nil, errors.InternalWithError("Failed to create membership", err)
	}
