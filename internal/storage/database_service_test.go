...
func TestDatabaseService_Create(t *testing.T) {
	...
	db, err := service.CreateDatabase("Test DB", columns)
	if err != nil {
...
func TestDatabaseService_CreateValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := service.CreateDatabase(tt.title, tt.columns)
...
func TestDatabaseService_Get(t *testing.T) {
	...
	created, err := service.CreateDatabase("Test DB", columns)
	if err != nil {
		t.Fatalf("Failed to create database: %v", err)
	}

	// Retrieve it
	got, err := service.GetDatabase(created.ID)
	if err != nil {
...
func TestDatabaseService_List(t *testing.T) {
	...
	for _, title := range titles {
		columns := []models.Column{{Name: "col", Type: "text"}}
		db, err := service.CreateDatabase(title, columns)
		if err != nil {
			t.Fatalf("Failed to create database: %v", err)
		}
...
func TestDatabaseService_Delete(t *testing.T) {
	...
	// Delete it
	err = service.DeleteDatabase(created.ID)
	if err != nil {
		t.Fatalf("Failed to delete database: %v", err)
	}

	// Verify it's gone
	_, err = service.GetDatabase(created.ID)
	if err == nil {
...
func TestDatabaseService_CreateRecord(t *testing.T) {
	...
	record, err := service.CreateRecord(db.ID, data)
	if err != nil {
...
func TestDatabaseService_GetRecords(t *testing.T) {
	...
	for i := 0; i < recordCount; i++ {
		...
		_, err := service.CreateRecord(db.ID, data)
		if err != nil {
			t.Fatalf("Failed to create record: %v", err)
		}
	}
	...
	// Get all records
	records, err := service.GetRecords(db.ID)
	if err != nil {
...
func TestDatabaseService_GetRecord(t *testing.T) {
	...
	created, err := service.CreateRecord(db.ID, data)
	if err != nil {
		t.Fatalf("Failed to create record: %v", err)
	}

	// Retrieve it
	got, err := service.GetRecord(db.ID, created.ID)
	if err != nil {
