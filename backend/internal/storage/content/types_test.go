...
func TestDataRecord(t *testing.T) {
	t.Run("Clone", func(t *testing.T) {
		...
		t.Run("nil data", func(t *testing.T) {
			original := &DataRecord{ID: rid.ID(1), Data: nil}
	...
	t.Run("GetID", func(t *testing.T) {
		if got := (&DataRecord{ID: rid.ID(42)}).GetID(); got != rid.ID(42) {
			t.Errorf("GetID() = %v, want %v", got, rid.ID(42))
		}
	})
	t.Run("Validate", func(t *testing.T) {
		t.Run("valid", func(t *testing.T) {
			if err := (&DataRecord{ID: rid.ID(1)}).Validate(); err != nil {
				t.Errorf("Validate() unexpected error: %v", err)
			}
		})
		t.Run("zero ID", func(t *testing.T) {
			if err := (&DataRecord{ID: rid.ID(0)}).Validate(); err == nil {
