...
func TestUniqueIndex_ExistingData(t *testing.T) {
	path := filepath.Join(t.TempDir(), "test.jsonl")
	table, err := NewTable[*testRow](path)
	if err != nil {
		t.Fatal(err)
...
func TestIndex(t *testing.T) {
	...
	// Test delete
	if _, err := table.Delete(ID(2)); err != nil {
		t.Fatal(err)
	}
	ids = collectIDs(byFirstLetter.Iter("a"))
	if len(ids) != 0 {
...
func TestIndex_ExistingData(t *testing.T) {
	path := filepath.Join(t.TempDir(), "test.jsonl")
	table, err := NewTable[*testRow](path)
	if err != nil {
		t.Fatal(err)
...
func TestIndex_IterEarlyTermination(t *testing.T) {
	path := filepath.Join(t.TempDir(), "test.jsonl")
	table, err := NewTable[*testRow](path)
	if err != nil {
		t.Fatal(err)
...
func TestUniqueIndex_UpdateSameKey(t *testing.T) {
	path := filepath.Join(t.TempDir(), "test.jsonl")
	table, err := NewTable[*testRow](path)
	if err != nil {
		t.Fatal(err)
...
func TestIndex_DeleteLastRowWithKey(t *testing.T) {
	path := filepath.Join(t.TempDir(), "test.jsonl")
	table, err := NewTable[*testRow](path)
	if err != nil {
		t.Fatal(err)
...
func TestIndex_UpdateRemovesLastRowFromOldKey(t *testing.T) {
	path := filepath.Join(t.TempDir(), "test.jsonl")
	table, err := NewTable[*testRow](path)
	if err != nil {
		t.Fatal(err)
