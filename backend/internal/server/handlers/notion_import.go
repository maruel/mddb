...
type NotionImportHandler struct {
	...
	mu     sync.Mutex
	states map[jsonldb.ID]*importState // wsID -> state
}
...
func NewNotionImportHandler(svc *Services, cfg *Config) *NotionImportHandler {
	return &NotionImportHandler{
		Svc:    svc,
		Cfg:    cfg,
		states: make(map[jsonldb.ID]*importState),
	}
}

// StartImport creates a new workspace and starts an async Notion import.
func (h *NotionImportHandler) StartImport(ctx context.Context, orgID jsonldb.ID, user *identity.User, req *dto.NotionImportRequest) (*dto.NotionImportResponse, error) {
...
func (h *NotionImportHandler) GetStatus(_ context.Context, orgID jsonldb.ID, _ *identity.User, req *dto.NotionImportStatusRequest) (*dto.NotionImportStatusResponse, error) {
...
func (h *NotionImportHandler) CancelImport(_ context.Context, wsID jsonldb.ID, _ *identity.User, _ *dto.NotionImportCancelRequest) (*dto.NotionImportCancelResponse, error) {
	h.mu.Lock()
	state := h.states[wsID]
	h.mu.Unlock()
...
func (h *NotionImportHandler) runImport(ctx context.Context, wsID jsonldb.ID, notionToken string, state *importState) {
	defer func() {
		if r := recover(); r != nil {
