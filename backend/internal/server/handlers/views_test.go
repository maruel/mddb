...
func setupTestServices(t *testing.T) (*Services, jsonldb.ID) {
	t.Helper()
	...
	if err := jsonldb.InitIDSlice(0, 1); err != nil {
		t.Fatal(err)
	}
...
func createTestTable(t *testing.T, svc *Services, wsID jsonldb.ID) jsonldb.ID {
	t.Helper()
	ctx := t.Context()
...
func TestViewCRUD(t *testing.T) {
	...
	// First add data
	record1 := &content.DataRecord{ID: jsonldb.NewID(), Data: map[string]any{"Name": "Alice", "Age": float64(25)}}
	record2 := &content.DataRecord{ID: jsonldb.NewID(), Data: map[string]any{"Name": "Bob", "Age": float64(10)}}
...
func TestListRecordsValidation(t *testing.T) {
	...
	t.Run("InvalidViewID", func(t *testing.T) {
		req := &dto.ListRecordsRequest{
			WsID:   wsID,
			ID:     nodeID,
			ViewID: jsonldb.NewID(), // Non-existent view
			Limit:  100,
		}
