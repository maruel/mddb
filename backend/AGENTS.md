...
- `docs/API.md`: mddb API Reference
...
- `internal/apiroutes/main.go`: Command apiroutes extracts API routes from router.go and generates docs/API.md.
...
- `internal/server/handlers/assets.go`: Handles file upload and retrieval for page assets.
...
- `internal/server/handlers/databases.go`: Handles table management endpoints.
...
- `internal/server/handlers/memberships.go`: Handles user context switching (org/workspace) and membership settings.
- `internal/server/handlers/nodes.go`: Handles hierarchical node operations (documents, tables, hybrid).
...
- `internal/server/handlers/pages.go`: Handles page management endpoints (documents).
