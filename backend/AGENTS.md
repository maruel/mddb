...
mddb uses [tygo](https://github.com/gzuidhof/tygo) to generate TypeScript interfaces from Go structs, and `github.com/maruel/apisdkgen` to generate the TypeScript client and API reference from the explicit route specification in `internal/server/dto/sdk.go`. apisdkgen is pinned in `go.mod`; the server test compares the route specification with the router registrations.
...
- `internal/cmd/gen-api-sdk/main.go`: Command gen-api-sdk generates mddb's TypeScript client and API reference.
...
- `internal/githubapp/client.go`: Manages GitHub App JWT generation and installation token caching.
- `internal/githubapp/client_test.go`: Tests for the GitHub App client.
- `internal/jsonldb/blob.go`: Defines the Blob type and content-addressed reference format.
- `internal/jsonldb/blob_test.go`: Tests for the JSONLDB blob type.
- `internal/jsonldb/blobstore.go`: Manages the on-disk storage layout for content-addressed blobs.
- `internal/jsonldb/blobstore_test.go`: Tests for the JSONLDB blob store.
- `internal/jsonldb/columns.go`: Handles schema definition, column types, and reflection-based schema generation.
- `internal/jsonldb/columns_test.go`: Tests for JSONLDB column serialization.
...
- `internal/jsonldb/index_test.go`: Tests for JSONLDB index operations.
- `internal/jsonldb/table.go`: Implements the concurrent-safe Table[T] for JSONL storage.
- `internal/jsonldb/table_test.go`: Tests for JSONLDB table CRUD.
...
- `internal/server/dto/errors_test.go`: Tests for DTO error constructors.
- `internal/server/dto/request.go`: Defines API request payloads and validation logic.
- `internal/server/dto/request_test.go`: Tests for API request validation logic.
- `internal/server/dto/response.go`: Defines API response payloads.
- `internal/server/dto/sdk.go`: SDK generation specification for mddb HTTP routes.
...
- `internal/server/gomode.go`: Go Mode discovery and authenticated, workspace-scoped MCP read and write access.
- `internal/server/gomode_test.go`: Tests authenticated Go Mode MCP reads and writes and embedded voice gateway access.
- `internal/server/handler_wrapper.go`: Provides middleware for standardizing HTTP handlers.
- `internal/server/handler_wrapper_benchmark_test.go`: Benchmarks the body-limit overhead for read-only multipart transports.
...
- `internal/server/handlers/assets_test.go`: Tests for asset serving handlers.
- `internal/server/handlers/auth.go`: Handles user authentication, registration, and session management.
- `internal/server/handlers/auth_test.go`: Tests for authentication handlers.
...
- `internal/server/handlers/errors_test.go`: Tests for error handling middleware.
- `internal/server/handlers/git_remotes.go`: Handles git remote configuration and synchronization.
- `internal/server/handlers/github_webhook.go`: Handles GitHub webhook events for sync-on-push.
- `internal/server/handlers/github_webhook_test.go`: Tests for GitHub webhook handler: signature verification and push event processing.
- `internal/server/handlers/health.go`: Handles health check endpoints.
- `internal/server/handlers/health_test.go`: Tests for the health check handler.
...
- `internal/server/handlers/nodes_test.go`: Tests for node/page API handlers.
- `internal/server/handlers/notifications.go`: Handles notification API endpoints.
- `internal/server/handlers/notify.go`: Central notification dispatcher called from existing handlers.
...
- `internal/server/handlers/oauth_testmode.go`: Provides a fake OAuth login handler for TEST_OAUTH=1 mode.
...
- `internal/server/handlers/sse.go`: SSE handler for streaming workspace events to connected clients.
- `internal/server/handlers/sse_test.go`: Tests for SSE event handlers.
- `internal/server/handlers/users.go`: Handles user management endpoints.
- `internal/server/handlers/views.go`: Handles view operations.
- `internal/server/handlers/views_test.go`: Tests for table views API handlers.
- `internal/server/integration_test.go`: Integration tests for the HTTP server.
- `internal/server/ipgeo/ipgeo.go`: Package ipgeo provides IP-to-country geolocation using MaxMind MMDB files.
- `internal/server/ipgeo/ipgeo_test.go`: Tests for IP geolocation lookup.
- `internal/server/ratelimit/config.go`: Defines rate limit tiers and routing rules.
- `internal/server/ratelimit/config_benchmark_test.go`: Benchmarks authenticated route classification for read-only MCP POST requests.
- `internal/server/ratelimit/config_test.go`: Tests for rate limit configuration.
- `internal/server/ratelimit/limiter.go`: Implements a thread-safe token bucket rate limiter.
- `internal/server/ratelimit/limiter_test.go`: Tests for the rate limiter.
- `internal/server/ratelimit/middleware.go`: Provides HTTP middleware and response writers for rate limiting.
- `internal/server/ratelimit/middleware_test.go`: Tests for rate limiting middleware.
- `internal/server/reqctx/context.go`: Defines request context keys and helper functions for metadata access.
- `internal/server/reqctx/context_test.go`: Tests for request context utilities.
- `internal/server/router.go`: Package server implements HTTP routing, middleware, and request handling.
- `internal/server/sdk_test.go`: SDK route coverage tests compare the generated client specification with registered JSON endpoints.
- `internal/server/sse/broker.go`: In-process pub/sub broker keyed by workspace ID for SSE event distribution.
- `internal/server/sse/broker_test.go`: Tests for the SSE broker.
- `internal/server/static.go`: Precompressed static file handler for embedded frontend assets.
- `internal/server/static_test.go`: Tests cache headers and SPA fallback for precompressed embedded frontend assets.
- `internal/server/voice_gateway.go`: Bounds embedded voice sessions and binds their signaling routes to the mddb user.
- `internal/server/voice_gateway_test.go`: Tests voice gateway admission limits and reservation cleanup.
- `internal/server/voice_smoke_test.go`: Live Gemini voice turn through the mddb embedded gateway and workspace MCP tools.
...
- `internal/storage/content/coercion_test.go`: Tests for content type coercion.
...
- `internal/storage/content/link_cache.go`: In-memory bidirectional link index for backlink queries.
...
- `internal/storage/content/table_benchmark_test.go`: Benchmarks for table operations.
- `internal/storage/content/types.go`: Defines the core data models for content (Node, DataRecord, Asset).
- `internal/storage/content/types_test.go`: Tests for content type helpers.
...
- `internal/storage/content/workspace_store_test.go`: Tests for the workspace store.
- `internal/storage/git/exec_repo.go`: Implements Repository using os/exec git commands.
- `internal/storage/git/git.go`: Defines the Repository interface, Manager, and shared types for git operations.
- `internal/storage/git/git_test.go`: Tests for Git operations.
- `internal/storage/git/gogit_repo.go`: Implements Repository using go-git (pure Go, no git binary dependency).
- `internal/storage/git/root_repo.go`: Manages the root data directory as a git repo with workspace submodules.
- `internal/storage/git/root_repo_test.go`: Tests for root repository management.
- `internal/storage/identity/email_verification.go`: Manages email verification tokens for magic link authentication.
- `internal/storage/identity/email_verification_test.go`: Tests for email verification.
- `internal/storage/identity/errors.go`: Defines sentinel errors for identity operations.
- `internal/storage/identity/invitation_test.go`: Tests for organization invitations.
- `internal/storage/identity/membership_test.go`: Tests for workspace membership.
- `internal/storage/identity/notification.go`: Manages notification entities and delivery preferences.
- `internal/storage/identity/notification_test.go`: Tests for user notifications.
...
- `internal/storage/identity/organization_test.go`: Tests for organization management.
- `internal/storage/identity/push_subscription.go`: Manages push subscription entities for web push notifications.
- `internal/storage/identity/push_subscription_test.go`: Tests for push notification subscriptions.
...
- `internal/storage/identity/user_test.go`: Tests for user account management.
...
- `internal/storage/quotas.go`: Defines shared resource quotas applied at server, organization, and workspace levels.
- `internal/storage/quotas_test.go`: Tests for quota enforcement.
- `internal/storage/static.go`: Embeds static files for the root data directory and workspace directories.
- `internal/storage/static/workspace/AGENTS.md`: Workspace Data Format
- `internal/storage/time.go`: Provides a JSON-friendly Time type with millisecond precision.
- `internal/syncsvc/sync.go`: Provides background sync operations (push/pull) between workspace git repos and remotes.
- `internal/syncsvc/sync_test.go`: Tests for sync service: acquire/release locking, getAuthURL, and push/pull error paths.
- `internal/utils/security.go`: Provides cryptographic utilities for token generation and hashing.
- `internal/utils/security_test.go`: Tests for security utilities.
- `tygo.yaml`: tygo configuration for Go-to-TypeScript type generation.
<!-- END FILE INDEX -->
